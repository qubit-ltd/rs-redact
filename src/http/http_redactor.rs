...
impl HttpRedactor {
    ...
    pub fn redact_headers_with_session(
        ...
        if !session.consume_input(input_bytes) {
            return RedactedHeaders::new(session_diagnostic_limit_exceeded(session));
        }
    ...
    fn redact_json(&self, bounded: &[u8], truncated: bool) -> ParsedBody {
        ...
        match json::serialize_bounded(&value, self.policy.body_budget().max_output_bytes()) {
        ...
        match std::str::from_utf8(bounded) {
            ...
            Ok(text) if is_text => match self.policy.text_body_policy() {
                ...
                TextBodyPolicy::PassThrough => {
                    ParsedBody::new(text.to_string(), BodyRedactionStatus::PassedThrough, false)
                }
            },
            Ok(_) => ParsedBody::new(
...
fn session_diagnostic_limit_exceeded(session: &RedactionSession<'_>) -> LogSafeText<'static> {
    match session.charge_output_or_fallback(
        markers::DIAGNOSTIC_LIMIT_EXCEEDED.len(),
        markers::DIAGNOSTIC_LIMIT_EXCEEDED.len(),
    ) {
        OutputCharge::Complete => HttpRedactor::diagnostic_limit_exceeded(),
        OutputCharge::Fallback | OutputCharge::Exhausted => empty_log_safe_text(),
    }
}
...
fn charge_header_output(session: &RedactionSession<'_>, value: RedactedHeaders) -> RedactedHeaders {
    match session.charge_output_or_fallback(
        value.log_safe_text().as_str().len(),
        markers::DIAGNOSTIC_LIMIT_EXCEEDED.len(),
    ) {
        OutputCharge::Complete => value,
        OutputCharge::Fallback => RedactedHeaders::new(HttpRedactor::diagnostic_limit_exceeded()),
        OutputCharge::Exhausted => RedactedHeaders::new(empty_log_safe_text()),
    }
}

/// Charges a safe body result to the shared session.
fn charge_body_output(session: &RedactionSession<'_>, value: BodyRedaction) -> BodyRedaction {
    match session.charge_output_or_fallback(
        value.log_safe_text().as_str().len(),
        markers::DIAGNOSTIC_LIMIT_EXCEEDED.len(),
    ) {
        ...
        OutputCharge::Exhausted => session_limited_body(BodyCapture::complete(b""), String::new()),
    }
}

/// Constructs a body result that contains no source bytes after a budget
/// failure.
fn session_limited_body(capture: BodyCapture<'_>, text: String) -> BodyRedaction {
    BodyRedaction::new(
        text,
