...
macro_rules! owned_map_key {
    ($key:ty) => {
        ...
        impl<V: super::RedactLevelValue + std::fmt::Debug> private::Sealed
            for Option<HashMap<$key, V>>
        {
        }
        impl<V: super::RedactLevelValue + std::fmt::Debug> RedactMapValue
            for Option<HashMap<$key, V>>
        {
            fn write_redacted_map(&self, fields: &mut RedactionFields<'_, '_>, name: &str) {
                match self {
...
macro_rules! borrowed_map_key {
    ($key:ty) => {
        impl<'a, V: super::RedactLevelValue + std::fmt::Debug> private::Sealed
            for HashMap<$key, V>
        {
        }
        ...
        impl<'a, V: super::RedactLevelValue + std::fmt::Debug> RedactMapValue
            for BTreeMap<$key, V>
        {
            fn write_redacted_map(&self, fields: &mut RedactionFields<'_, '_>, name: &str) {
                fields.map_entries(name, self.iter());
