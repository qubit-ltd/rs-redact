    ...
    for<'entry> &'entry M: IntoIterator<Item = (&'entry K, &'entry V), IntoIter: ExactSizeIterator>,
{
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        ...
        let view = RedactedKeyedMapResult::new_with_alternate(self.map, &mut session, formatter.alternate());
...
mod session_view {
    ...
    impl<'map, M: ?Sized, K: AsRef<str> + Debug + ?Sized + 'map, V: Redact + RedactValue + ?Sized + 'map>
    ...
    impl<'map, M: ?Sized, K: AsRef<str> + Debug + ?Sized + 'map, V: Redact + RedactValue + ?Sized + 'map> Debug
        for KeyedMapOnce<'map, '_, '_, M, K, V>
    where
        for<'entry> &'entry M: IntoIterator<Item = (&'entry K, &'entry V)>,
        <&'map M as IntoIterator>::IntoIter: ExactSizeIterator,
    {
        ...
        fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
            ...
            loop {
                ...
                let Some(view) =
                    RedactedKeyedResult::try_new_admitted_item(key.as_ref(), value, scope.session(), alternate)
                else {
                    output.entry(&DomainTruncated, &DomainTruncated);
                    break;
                };
