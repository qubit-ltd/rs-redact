...
impl Serialize for SerializerProbe {
        ...
        S: Serializer,
    {
        match self {
            ...
            Self::NewtypeStruct => serializer.serialize_newtype_struct("Inner", &serde_json::json!({"value": 1})),
            Self::NewtypeVariant => {
                serializer.serialize_newtype_variant("Inner", 0, "inner", &serde_json::json!({"value": 1}))
            }
            Self::Seq => {
...
fn test_internal_tagged_serializer_covers_supported_shapes() {
    let cases = [
        (SerializerProbe::Unit, serde_json::json!({"kind": "Payload"})),
        (SerializerProbe::UnitStruct, serde_json::json!({"kind": "Payload"})),
        (
...
fn test_internal_tagged_serializer_propagates_destination_errors() {
    ...
    for probe in cases {
        for remaining in 0..64 {
            ...
            let _ = serialize_internally_tagged(&mut serializer, "Event", "Payload", "kind", "Payload", &probe);
        }
    }
}
