...
fn alternate_log_safe_text(headers: &RedactedHeaders) -> &RedactedText {
    headers.log_safe_text()
}

/// Alternate consuming query used as an unselected function target.
fn alternate_into_log_safe_text(headers: RedactedHeaders) -> RedactedText {
...
fn test_redacted_headers_hides_authorization_value() {
    ...
    let borrowed_text: [for<'a> fn(&'a RedactedHeaders) -> &'a RedactedText; 2] =
        [RedactedHeaders::log_safe_text, alternate_log_safe_text];
    let owned_text: [fn(RedactedHeaders) -> RedactedText; 2] =
