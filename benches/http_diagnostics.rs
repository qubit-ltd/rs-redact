...
use qubit_redact::http::{
    BodyBudget, BodyCapture, DiagnosticBudget, HttpRedactionPolicy, HttpRedactor,
};
...
fn benchmark_diagnostic_budgets(criterion: &mut Criterion) {
    ...
    let redactor = redactor_with_diagnostic_budget(
        DiagnosticBudget::new(INPUT_LIMIT, 512).expect("benchmark diagnostic budget is valid"),
    );
    let sizes = [
        ("below", INPUT_LIMIT / 4),
        ("near", INPUT_LIMIT - 64),
