...
pub(crate) fn add_redact_bounds(
    ...
    for_each_field(model, &mut |field, mode, _serialize_with| match mode {
        ...
        FieldMode::DisplayLevel(_) => {
            add_trait_bound(generics, field, quote!(::core::fmt::Display))
        }
        FieldMode::Level(_) => {
            add_trait_bound(generics, field, quote!(#runtime::domain::RedactLevelValue));
        }
        ...
        FieldMode::MapLevels { .. } => {
            add_trait_bound(generics, field, quote!(#runtime::domain::RedactMapKeyValue))
        }
        FieldMode::Json => {
            add_trait_bound(generics, field, quote!(#runtime::domain::RedactJsonValue))
        }
        FieldMode::Skip => {}
    });
}
